...
export function activate(ctx: vscode.ExtensionContext): void {
    ...
    vscode.languages.registerDocumentFormattingEditProvider('proto3', {
        provideDocumentFormattingEdits(document: vscode.TextDocument): vscode.ProviderResult<vscode.TextEdit[]> {
            ...
            switch (Proto3Configuration.Instance().formatter()) {
                case 'clang-format':
                    let args = [];
                ...
                case 'buf':
                    return document.save().then(() => {
                        ...
                        try {
                            let stdout = cp.execFileSync('buf', ["format", document.uri.fsPath]);
                            let edit = [new vscode.TextEdit(document.validateRange(new vscode.Range(0, 0, Infinity, Infinity)), stdout ? stdout.toString() : '')];
